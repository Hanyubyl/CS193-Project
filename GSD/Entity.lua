...
function Movement:setveloup(velo)
		self.velo.y = - velo
end

function Movement:setvelodown(velo)

		self.velo.y = velo

end

function Movement:setveloleft(velo)

		self.velo.x =- velo

end

function Movement:setveloright(velo)

		self.velo.x = velo

end
function Movement:velodir(velo, vec, neg)
	neg = neg or 1
	if vec.x == 1 then
		return self:veloleft(velo*neg)
	end
	if vec.x == -1 then
		return self:veloright(velo*neg)
	end
	if vec.y == 1 then
		return self:veloup(velo*neg)
	end
	if vec.y == -1 then
		return  self:velodown(velo*neg)
	end
end

function Movement:kb_velodir(velo, vec, neg)
	neg = neg or 1
	if vec.x == 1 then
		self.kb_velo.x = -velo
	end
...
function Movement:kill_velo()

	self.velo.x = 0
	self.velo.y = 0
end

function Movement:kill_kb_velo()
...
------------------------------------------------------
Keys = {}
function Keys:keys_init(k)
	self.keys = k or 0
end

function Keys:keys_add()
	self.keys = self.keys + 1
end

function Keys:keys_sub()
	if(self.keys > 0) then
		self.keys = self.keys - 1
		return self.keys
	else -- returning 0 or less means not enough keys
		return self.keys
	end
end

function Keys:keys_add_amount(k)
	self.keys = self.keys + k
end

function Keys:keys_sub_amount(k)
		if(self.keys-k > -1) then
			self.keys = self.keys - k
		return self.keys
	else -- returning 0 or less means not enough keys
		self.keys = 0
...
function Health_mana:zero_health()
	self.hp_current = 0
end
...
function Health_mana:zero_mana()
	self.mp_current = 0
end
...
function Health_mana:sub_hp(sub)	
	-- following line is for type-casting purposes
	--sub = sub1 - 0
	if self.damage_immune then
		return
	elseif sub > self.hp_current then
		self.hp_current = 0
	else
		self.hp_current = self.hp_current - sub
		print(self.hp_current)		
		function es()
		...
		self:dttimer_add(dttimer:new(es,ee,.5))
	end
