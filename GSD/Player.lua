...
function Player:initialize(world, x, y, z, settings)
	...
	self:health_mana_init(1000,100)
	self:keys_init()
...
function Player:update(dt)
	...
	-- map collision
		...
		local future_x = self.x + (self.velo.x*self.envx + self.kb_velo.x)*dt 
		local future_y = self.y + (self.velo.y*self.envy + self.kb_velo.y)*dt
